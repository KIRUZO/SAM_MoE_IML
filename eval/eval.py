...
class MoE_Adapter(nn.Module):
    def __init__(self, original_linear, num_shared=1, num_routed=6, top_k=2, rank=16):
        ...
        self.num_shared = num_shared; self.num_routed = num_routed; self.top_k = top_k
    ...
    def forward(self, x):
        with torch.no_grad(): base_out = self.original_linear(x)
        shared_out = 0
        for expert in self.shared_experts: shared_out += expert(x)
...
if __name__ == "__main__":
    ...
    model_name = "SAM_RAG_MoE_Learnable"
